...
const actions = {
	deleteThread ({ state }, vue) {
		vue.axios
			.put(baseUrl + '/api/v1/thread/' + state.threadId)
			.then(() => {
				vue.$router.push('/')
			})
	...
	removePostsAsync ({ state, commit }, vue) {
		...
		let promises = state.selectedPosts.map(id => vue.axios.put(baseUrl + '/api/v1/post/' + id))
	...
	loadInitialPostsAsync ({ commit, dispatch }, vue) {
		commit('setPosts', [])
		commit('setThread', { name: 'Загрузка...' })
		dispatch('setTitle', 'Загрузка...')
	...
	setThreadLockedState ({ state, commit }, vue) {
		vue.axios
			.put(baseUrl + '/api/v1/thread/' + state.threadId, { locked: !state.locked })
			.then(() => {
