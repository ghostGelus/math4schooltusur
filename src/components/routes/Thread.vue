<template>
	...
	<div class='route_container' :style='posts.length ? "padding-bottom: 8.5rem;" : null'>
		<confirm-modal v-model='showConfirmModal' @confirm='deleteThread' text='Delete' color='red'>
			...
			<br>Этот пост <b>нельзя</b> будет восстановить
		</confirm-modal>
		...
		<div class='thread_side_bar'>
			...
			>
				Удалить ({{$store.state.thread.selectedPosts.length}})
			</loading-button>
			<menu-button
				v-if='$store.state.admin'
				:options='[
					{ event: "Заблокировать", value: $store.state.thread.locked ? "Unlock thread" : "Lock thread" },
					{ event: "Удалить пост", value: "Delete thread" },
					{ event: "Удалить комментарии", value: "Remove posts" }
				]'
		...
		<div class='locked_thread' v-if='$store.state.thread.locked'>
			<h2>Пост заблокирован </h2>
			Вы не можете оставлять комментарии в этой теме, потому что она была заблокирована администратором
		</div>
