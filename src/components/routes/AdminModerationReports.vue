<template>
	<div class='admin_moderation'>
		...
		<confirm-modal v-model='removePostObj.showConfirmModal' @confirm='removePost' text='Удалить' color='red'>
			Вы действительно хотите удалить комментарий?
		</confirm-modal>

		<confirm-modal v-model='removePostObj.showThreadDeleteModal' @confirm='deleteThread' text='Удалить' color='red'>
		...
		<transition name='fade' mode='out-in'>
			...
			>
				...
				<div class='admin_moderation__report admin_moderation__report--header'>
					...
					<div class='admin_moderation__report__reason admin_moderation__report--cell_border admin_moderation__report--cell_border-hidden'>Причина </div>
				...
				<div class='admin_moderation__report' :key='"report-" + $index' v-for='(report, $index) in filteredReports'>
					...
					<div class='admin_moderation__report__actions'>
						<button class='button button--red' @click='removePost(report, $index)'>Удалить</button>
						<menu-button
						...
						>
							<button class='button'>Заблокировать&hellip;</button>
						</menu-button>
					</div>
				</div>
			</div>
...
<script>
	...
	export default {
		...
		data () {
			return {
				reportMenuOptions: [
					{ value: "Заблокировать пользователя", event: 'ban' },
					{ value: "Удалить пост", event: 'deleteThread' }
				],
				reports: null,
		...
		methods: {
			...
			deleteThread (report, index) {
				if(report) {
					...
					this.axios
						.put(baseUrl + '/api/v1/thread/' + threadId)
						.then(() => {
			...
			removePost (report, index) {
				if(report) {
					...
					this.axios
						.put(baseUrl + '/api/v1/post/' + this.removePostObj.report.Post.id)
						.then(() => {
							return this.axios.put(baseUrl + '/api/v1/report/' + this.removePostObj.report.id)
						})
						.then(() => {
