<template>
	...
	<div class='route_container thread_new'>
		...
		<div class='h1'>Опубликуйте новый пост</div>
		<div class='thread_meta_info'>
			<div class='thread_meta_info__text'>Выберите категорию и введите название поста</div>
				...
				<div class='thread_meta_info__poll' v-if='showPoll'>
					...
					<div>
						...
						>
							...
							></fancy-input>
							<span @click='removePollAnswer($index)' title='Удалить ответ'>&times;</span>
						</div>
						<div class='thread_meta_info__form'>
							<fancy-input
								v-model='newPollAnswer'
								placeholder='Вариант ответа на опрос'
								style='display: inline-block; margin-right: 0.5rem;'
								width='15rem'
								:large='true'
								:error='errors.pollAnswer'
							></fancy-input>
							<button class='button button--thin_text' @click='addPollAnswer'>Добавить ответ</button>
						</div>
					</div>
				</div>
			</transition>
		...
		<loading-button class='button--green submit' :loading='loading' @click='postThread'>Опубликовать пост</loading-button>
	</div>
	</div>
</template>
